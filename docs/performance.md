...
v1モデルの精度が[v1時の報告](docs/performance_v1.md#備考-転移学習に用いたデータに対する識別性能)と異なっているのは、chainerのバージョンを変更したところ、v1モデルの精度が以前より向上したためである。
